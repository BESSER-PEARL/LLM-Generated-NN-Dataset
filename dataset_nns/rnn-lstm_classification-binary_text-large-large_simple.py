...
class TextLSTMClassifier(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=128, hidden_size=96, num_layers=2, batch_first=True, dropout=0.2, bidirectional=True)
        self.dropout = nn.Dropout(p=0.3)
        self.fc = nn.Linear(in_features=192, out_features=1)
    def forward(self, x):
        x = self.embedding(x)
        out, _ = self.lstm(x)
        x = out[:, -1, :]
