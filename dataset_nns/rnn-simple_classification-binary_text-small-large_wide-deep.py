...
class BinaryTextRNN(nn.Module):
    def __init__(self):
        super().__init__()
        self.embedding = nn.Embedding(num_embeddings=100001, embedding_dim=128, padding_idx=0)
        self.rnn = nn.RNN(input_size=128, hidden_size=96, num_layers=3, nonlinearity='tanh', batch_first=True, dropout=0.2, bidirectional=False)
