...
class RNNBinaryTextClassifier(nn.Module):
    def __init__(self):
        super().__init__()
        self.embedding = nn.Embedding(num_embeddings=120000, embedding_dim=128, padding_idx=0)
        self.dropout = nn.Dropout(p=0.2)
        self.rnn = nn.RNN(input_size=128, hidden_size=256, num_layers=1, nonlinearity='tanh', batch_first=True, bidirectional=False)
